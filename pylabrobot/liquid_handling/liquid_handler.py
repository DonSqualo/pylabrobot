...
class LiquidHandler(MachineFrontend):
    ...
    def __init__(self, backend: LiquidHandlerBackend, deck: Deck):
        ...
        self.deck.resource_unassigned_callback_callback = (
            self.resource_unassigned_callback
        )
    ...
    def unassign_resource(self, resource: Union[str, Resource]):  # TODO: remove this.
        ...
        if r is None:
            raise KeyError(
                f"Resource '{resource}' is not assigned to this liquid handler."
            )
        r.unassign()
        ...
        try:
            ...
            for channel, op in zip(use_channels, drops):
                if does_tip_tracking() and (
                    isinstance(op.resource, TipSpot)
                    and not op.resource.tracker.is_disabled
                ):
                    op.resource.tracker.rollback()
                self.head[channel].rollback()
    ...
    async def aspirate(
        ...
        **backend_kwargs,
        ...
        if isinstance(resources, Resource):  # if single resource, space channels evenly
            ...
            if offsets is not None:
                if not isinstance(offsets, list) or len(offsets) != n:
                    raise ValueError(
                        "Number of offsets must match number of channels used when aspirating "
                        "from a resource."
                    )
            else:
        ...
        aspirations = [
            ...
            for r, v, o, fr, lh, t, lvs in zip(
                resources, vols, offsets, flow_rates, liquid_height, tips, liquids
            )
        ]
        ...
        **backend_kwargs,
        ...
        if isinstance(resources, Resource):  # if single resource, space channels evenly
            ...
            if offsets is not None:
                if not isinstance(offsets, list) or len(offsets) != n:
                    raise ValueError(
                        "Number of offsets must match number of channels used when dispensing "
                        "to a resource."
                    )
            else:
                offsets = resources.get_2d_center_offsets(n=n)
    ...
    async def transfer(
        ...
        **backend_kwargs,
        ...
        assert use_backend_kwargs in [
            "only_on_aspirate",
            "only_on_dispense",
            "both",
        ], ValueError(
            "use_backend_kwargs must be one of 'only_on_aspirate', 'only_on_dispense', or 'both'"
        )
        ...
        if isinstance(sources, Well) and isinstance(targets, Well):
            ...
            assert (source_vol is not None) ^ (target_vols is not None), ValueError(
                "1 to 1: Must specify source_vol or target_vols, but not both, got",
                source_vol,
                target_vols,
            )
            assert (
                source_vol is None
                or isinstance(source_vol, numbers.Rational)
                or len(source_vol) == 1
            ), ValueError(
                "1 to 1: source_vol must have only one value, got", source_vol
            )
            assert (
                target_vols is None
                or isinstance(target_vols, numbers.Rational)
                or len(target_vols) == 1
            ), ValueError(
                "1 to 1: target_vols must have only one value, got", target_vols
            )
            assert ratios is None, ValueError(
                "1 to 1: Cannot specify ratios, got", ratios
            )
            ...
            assert (
                aspiration_flow_rates is None
                or isinstance(aspiration_flow_rates, int)
                or len(aspiration_flow_rates) == 1
            ), ValueError(
                "1 to 1: aspiration_flow_rates must have only one value, got",
                aspiration_flow_rates,
            )
            ...
            assert (
                dispense_flow_rates is None
                or isinstance(dispense_flow_rates, int)
                or len(dispense_flow_rates) == 1
            ), ValueError(
                "1 to 1: dispense_flow_rates must have only one value, got",
                dispense_flow_rates,
            )
            ...
            assert (
                use_channels is None
                or isinstance(use_channels, int)
                or len(use_channels) == 1
            ), ValueError(
                "1 to 1: use_channels must have only one value, got", use_channels
            )
        ...
        if isinstance(sources, Well) and isinstance(targets, list):
            # source_vol, target_vol and ratios:
            assert (source_vol is not None) ^ (target_vols is not None), ValueError(
                "1 to many: Must specify source_vol or target_vols, but not both, got",
                source_vol,
                target_vols,
            )
            assert (
                ...
                "1 to many: source_vol must have only one value, got", source_vol
            )
            assert (
                target_vols is None
                or isinstance(target_vols, numbers.Rational)
                or len(target_vols) == len(targets)
            ), ValueError(
                "1 to many: target_vols must be a number or have same length as targets, got",
                target_vols,
            )
            ...
            source_vol = (
                source_vol if source_vol is not None else sum_mixed(target_vols)
            )
            ...
            assert (
                dispense_flow_rates is None
                or isinstance(dispense_flow_rates, numbers.Rational)
                or len(dispense_flow_rates) == len(targets)
            ), ValueError(
                "1 to 1: dispense_flow_rates must be a number or a list with the same length as targets, got",
                dispense_flow_rates,
            )
            ...
            assert (
                ...
                "1 to many: use_channels must have only one value, got", use_channels
            )
            if isinstance(use_channels, list):
                use_channels = use_channels[0]
            if use_channels is None:
            ...
            for target, vol, disp_flow in zip(
                targets, target_vols, dispense_flow_rates
            ):
                await self.dispense(
                    resources=[target],
                    vols=vol,
        ...
        if isinstance(sources, list) and isinstance(targets, Well):
            # source_vol, target_vol and ratios:
            assert (source_vol is not None) ^ (target_vols is not None), ValueError(
                "many to 1: Must specify source_vol or target_vols, but not both, got",
                source_vol,
                target_vols,
            )
            assert (
                source_vol is None
                or isinstance(source_vol, numbers.Rational)
                or len(source_vol) == len(sources)
            ), ValueError(
                "many to 1: source_vol must have same length as sources, got",
                source_vol,
            )
            assert (
                target_vols is None
                ...
                "many to 1: target_vols must have only one value, got", target_vols
            )

            # first target_vols: get or calculate from sources and convert to list of length 1
            target_vols = (
                target_vols if target_vols is not None else self.sum_mixed(source_vol)
            )
            if isinstance(target_vols, numbers.Rational):
            ...
            assert (
                aspiration_flow_rates is None
                or isinstance(aspiration_flow_rates, int)
                or len(aspiration_flow_rates) == len(sources)
            ), ValueError(
                "many to 1: aspiration_flow_rates must be a number or a list with the same length as sources, got",
                aspiration_flow_rates,
            )
            if isinstance(aspiration_flow_rates, int):
            ...
            assert (
                dispense_flow_rates is None
                or isinstance(dispense_flow_rates, int)
                or len(dispense_flow_rates) == len(sources)
            ), ValueError(
                "many to 1: dispense_flow_rates must be a number or a list with the same length as sources, got",
                dispense_flow_rates,
            )
            if isinstance(dispense_flow_rates, int):
            ...
            assert (
                use_channels is None
                or isinstance(use_channels, int)
                or (len(use_channels) == len(sources) and len(use_channels) <= 8)
            ), ValueError(
                "many to 1: use_channels must have same length as sources and less than 8, got",
                use_channels,
            )
            ...
            if len(use_channels) == 1:
                for source, vol, asp_flow in zip(
                    sources, source_vol, aspiration_flow_rates
                ):
                    await self.aspirate(
                        resources=[source],
                        vols=vol,
            ...
            # then dispense each tip into the same target
                ...
                for channel, target, vol, disp_flow in zip(
                    use_channels, targets, target_vols, dispense_flow_rates
                ):
                    await self.dispense(
                        resources=[target],
                        vols=vol,
        ...
        if isinstance(sources, list) and isinstance(targets, list):
            # source_vol, target_vol and ratios:
            assert source_vol is not None or target_vols is not None, ValueError(
                "many to many: Must specify source_vol or target_vols, got",
                source_vol,
                target_vols,
            )
            assert source_vol is None or (
                isinstance(source_vol, list) and len(source_vol) == len(sources)
            ), ValueError(
                "many to many: source_vol must have same length as sources, got",
                source_vol,
            )
            assert target_vols is None or (
                isinstance(target_vols, list) and len(target_vols) == len(targets)
            ), ValueError(
                "many to many: target_vols must have same length as targets, got",
                target_vols,
            )
            ...
            if source_vol is not None and target_vols is not None:
                ...
                assert not isinstance(source_vol, numbers.Rational) and isinstance(
                    target_vols, numbers.Rational
                ), ValueError(
                    "many to many: source_vol and target_vols cannot both be numbers, got",
                    source_vol,
                    target_vols,
                )
                ...
                if isinstance(source_vol, list) and isinstance(target_vols, list):
                    assert sum(source_vol) == sum(target_vols), ValueError(
                        "many to many: source_vol and target_vols must have the same sum, got",
                        source_vol,
                        target_vols,
                    )
                    source_vol = source_vol
                ...
                if isinstance(source_vol, numbers.Rational) and isinstance(
                    target_vols, list
                ):
                    assert source_vol == sum(target_vols), ValueError(
                        "many to many: source_vol and target_vols must have the same sum, got",
                        source_vol,
                        target_vols,
                    )
                ...
                if isinstance(source_vol, list) and isinstance(
                    target_vols, numbers.Rational
                ):
                    assert sum(source_vol) == target_vols, ValueError(
                        "many to many: source_vol and target_vols must have the same sum, got",
                        source_vol,
                        target_vols,
                    )
            ...
            assert (
                ...
                or isinstance(aspiration_flow_rates, int)
                or (
                    len(aspiration_flow_rates) <= 8
                    and len(aspiration_flow_rates) == len(sources)
                )
            ), ValueError(
                "many to many: aspiration_flow_rates must be a number or a list with the same length as sources and less than 8, got",
                aspiration_flow_rates,
            )
            if isinstance(aspiration_flow_rates, int):
                aspiration_flow_rates = [aspiration_flow_rates] * len(sources)
            ...
            assert (
                ...
                or isinstance(dispense_flow_rates, int)
                or (
                    len(dispense_flow_rates) <= 8
                    and len(dispense_flow_rates) == len(targets)
                )
            ), ValueError(
                "many to many: dispense_flow_rates must be a number or a list with the same length as targets and less than 8, got",
                dispense_flow_rates,
            )
            if isinstance(dispense_flow_rates, int):
                dispense_flow_rates = [dispense_flow_rates] * len(targets)
            ...
            assert use_channels is None or (
                isinstance(use_channels, list)
                and (len(use_channels) == len(sources))
                or (len(use_channels) == len(targets))
            ), ValueError(
                "many to many: use_channels must have same length as sources or targets, got",
                use_channels,
            )

            assert round(sum(source_vol), 1) == round(sum(target_vols), 1), ValueError(
                "many to many: something went wrong in the calculation of source and target volumes, got",
                source_vol,
                target_vols,
            )
    ...
    async def pick_up_tips96(
        ...
        **backend_kwargs,
        ...
        await self.backend.pick_up_tips96(
            pickup=PickupTipRack(resource=tip_rack, offset=offset), **backend_kwargs
        )
    ...
    async def drop_tips96(
        ...
        **backend_kwargs,
        ...
        await self.backend.drop_tips96(
            drop=DropTipRack(resource=tip_rack, offset=offset), **backend_kwargs
        )
        ...
        **backend_kwargs,
        ...
        if plate.num_items_x == 12 and plate.num_items_y == 8:
            ...
            raise NotImplementedError(
                f"It is not possible to plate aspirate from an {plate.num_items_x} "
    ...
    async def dispense_plate(
        ...
        **backend_kwargs,
        ...
        if plate.num_items_x == 12 and plate.num_items_y == 8:
            ...
            raise NotImplementedError(
                f"It is not possible to plate dispense to an {plate.num_items_x} "
                f"by {plate.num_items_y} plate"
            )
    ...
    async def stamp(
        ...
        await self.aspirate_plate(
            plate=source, volume=volume, flow_rate=aspiration_flow_rate
        )
        await self.dispense_plate(
            plate=source, volume=volume, flow_rate=dispense_flow_rate
        )
    ...
    async def move_resource(
        ...
        **backend_kwargs,
        ...
        return await self.backend.move_resource(
            move=Move(
                resource=resource,
                destination=to,
    ...
    async def move_lid(
        ...
        **backend_kwargs,
        ...
        if isinstance(to, Plate):
            ...
            to_location = to.get_absolute_location()
            to_location = Coordinate(
                x=to_location.x, y=to_location.y, z=to_location.z + to.get_size_z()
            )
    ...
    def serialize(self) -> dict:
        ...
        return {"deck": self.deck.serialize(), "backend": self.backend.serialize()}
